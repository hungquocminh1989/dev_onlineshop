...
class ACWView
{
	...
	//Add Start - LIXD-427 - TrungVNIT - 2016/01/20
	public static function download_file2($file_name, $file_path, $delete = FALSE)
	{
		if (strpos($_SERVER['HTTP_USER_AGENT'], "MSIE") === false) {
			// IEの文字化け対策
		} else {
		...
		if(file_exists($file_path) === FALSE){
			...
			if($fileLib->FileExists($file_path) === TRUE){
				ACWLog::write_file('LIXD-427', '---- DOWNLOAD WINDOWS LIB ----'.PHP_EOL.'FileName: '.$file_name.PHP_EOL.'FilePath: '.$file_path);
				if($fileLib->FolderExists(AKAGANE_DL_PATH_TMP) == FALSE){
					$fileLib->CreateFolder(AKAGANE_DL_PATH_TMP);
				}
				$ext = $fileLib->GetExtensionName($file_path);
				$download_path = AKAGANE_DL_PATH_TMP . '/' . uniqid(time()) . '.' . $ext;
				$fileLib->CopyFile($file_path, $download_path);
				readfile($download_path);
				$fileLib->DeleteFile($download_path);
				if($delete === TRUE){
					$fileLib->DeleteFile($file_path);
				}
			}
		}else{
			ACWLog::write_file('LIXD-427', '---- DOWNLOAD NORMAL ----'.PHP_EOL.'FileName: '.$file_name.PHP_EOL.'FilePath: '.$file_path);
			readfile($file_path);
			if($delete === TRUE){
				unlink($file_path);
			}
	...
	//Add End - LIXD-427 - TrungVNIT - 2016/01/20
