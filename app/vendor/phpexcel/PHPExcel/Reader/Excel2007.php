		...
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if (!$this->_readDataOnly && $xmlStyles) {
						...
						if ($xmlStyles->dxfs) {
							foreach ($xmlStyles->dxfs->dxf as $dxf) {
								...
								$dxfs[] = $style;
								$style->Destroyer(); //add LIXD-287 Phong VNIT-20151030
							}
						}
						//	Cell Styles
					...
					if ($xmlWorkbook->sheets) {
						//foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {  //Edit LIXD-284 hungtn VNIT 20151027
                            $eleSheet = $xmlWorkbook->sheets->sheet[0];//Edit LIXD-284 hungtn VNIT 20151027
							++$oldSheetId;
...
// TODO: Autoshapes from twoCellAnchors!
							...
							// Next sheet id
							++$sheetId;
						//}//Edit LIXD-284 hungtn VNIT 20151027
							...
							foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
								...
								if ((string)$definedName['localSheetId'] != '') {
									...
									switch ((string)$definedName['name']) {
										...
										default:
											if (isset($mapSheetId[(integer) $definedName['localSheetId']]) && $mapSheetId[(integer) $definedName['localSheetId']] !== null) { //BP_LIXD-669 Tin add isset
									...
									if (strpos( (string)$definedName, '!' ) !== false) {
										...
										// matsuyama
										$multiRange = explode(',', $extractedRange);
										if (count($multiRange) == 1) {
											// Modify range
											...
											$cm = '';
											$extractedRange = '';
											foreach ($multiRange as $range1) {
												$extractedRange .= $cm;
												$range = explode('!', $range1);
												$extractedRange .= isset($range[1]) ? $range[1] : $range[0];
												$cm = ',';
											}
										}
