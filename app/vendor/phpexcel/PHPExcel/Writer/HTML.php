...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	...
	public function generateSheetData() {
		...
		foreach ($sheets as $sheet) {
			...
			$dimension = explode(':', $sheet->calculateWorksheetDataDimension()); //Edit LIXD-334 TinVNIT 11122015
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		foreach ($this->_phpExcel->getCellXfCollection() as $index => $style) {
			...
			//add start LIXD-79 Phong VNIT-20150910
			if($style->getAlignment()->getTextRotation() != 0){
				$css['td.style' . $index.' span'] =$this->_createCSSSTransform($style->getAlignment());
			}
			//add end LIXD-79 Phong VNIT-20150910
		...
		$countSheet = 0; //Add BP_LIXD-667 TinVNIT 04212017
		// Build styles per sheet
		foreach ($sheets as $sheet) {
			//Add start BP_LIXD-667 TinVNIT 04212017
			$countSheet++;
			if($countSheet == 1)
			{				
				$merge_cells = $sheet->getMergeCells();
				if(count($merge_cells) > 0)
				{
					foreach ($merge_cells as $merge_cell) {

						// 変換① 結合単位からセル単位 A1:A2 → A1, A2
						$pos = explode(':', $merge_cell);
						
						// 変換② セル単位からセル位置
						$pos_s = $pos[0];
						$pos_e = $pos[1];
						$cssClassStart = 'td.style' . $sheet->getCell($pos_s)->getXfIndex();
						$cssClassEnd = 'td.style' . $sheet->getCell($pos_e)->getXfIndex();
						$id_merge = 'td#merge_'.$pos_s.'_'.$pos_e;
						$css[$id_merge] = array();
						$border_merge_left = $css[$cssClassStart]['border-left'];
						if(strpos($border_merge_left, '!important') == false)
						{
							$border_merge_left = $border_merge_left . ' !important';
						}
						
						$border_merge_top = $css[$cssClassStart]['border-top'];
						if(strpos($border_merge_top, '!important') == false)
						{
							$border_merge_top = $border_merge_top . ' !important';
						}
						
						$border_merge_bottom = $css[$cssClassEnd]['border-bottom'];
						if(strpos($border_merge_bottom, '!important') == false)
						{
							$border_merge_bottom = $border_merge_bottom . ' !important';
						}
						
						$border_merge_right = $css[$cssClassEnd]['border-right'];
						if(strpos($border_merge_right, '!important') == false)
						{
							$border_merge_right = $border_merge_right . ' !important';
						}
						
						$css[$id_merge]['border-left'] = $border_merge_left;						
						$css[$id_merge]['border-top'] = $border_merge_top;
						$css[$id_merge]['border-bottom'] = $border_merge_bottom;
						$css[$id_merge]['border-right'] = $border_merge_right;						
					}
				}
				//Add end BP_LIXD-667 TinVNIT 04212017
			}
			...
			$highestColumnIndex = PHPExcel_Cell::columnIndexFromString($sheet->getHighestDataColumn()) - 1;
			$column = -1;
			   $allpt = array();
				while($column++ < $highestColumnIndex) {
				$this->_columnWidths[$sheetIndex][$column] = 42; // approximation
				$css['table.sheet' . $sheetIndex . ' col.col' . $column]['width'] = '42pt';
				$allpt['col' . $column] = 42;	// matsuyama
				}

			// col elements, loop through columnDimensions and set width
			$countColumnDimension = 0;  //Add LIXD-334 TinVNIT 11122015
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				//Add start LIXD-334 TinVNIT 11122015
				$countColumnDimension++; 
				if($countColumnDimension > $highestColumnIndex)
					break;
				//Add end LIXD-334 TinVNIT 11122015
				if (($width = PHPExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
					...
					$allpt['col' . $column] = $width;	// matsuyama
			...
			$tablewidth = 0;
			foreach ($allpt as $akey => $aval) {
				$tablewidth += $aval * 1.2;
			}

			$css['table']['width'] = $tablewidth . 'pt';//matuyama
	...
	private function _createCSSStyle(PHPExcel_Style $pStyle) {
		...
		$css = array_merge(
			...
			,$this->_createCSSSDiagonDirection($pStyle->getBorders())//add LIXD-79 Phong VNIT-20150911
		);
	...
	//add start LIXD-79 Phong VNIT-20150910
	private function _createCSSSTransform(PHPExcel_Style_Alignment $pStyle) {
		$css = array();
		
		$textAlign = $this->_mapHAlign($pStyle->getHorizontal());	
		$vertical = $this->_mapVAlign($pStyle->getVertical());	
		...
		if( $rotation == -165){
		    ...
		    // $css['font-family']= 'block';		    
		    //$css['white-space']= 'pre';		    
		}else if($rotation == -90){			
    		$rotation =-1* $rotation;    		
			$css['display']='block';	
			$css['transform'] = 'rotate('.$rotation.'deg)';			
			if ( $textAlign =='center' && $vertical=='middle') {
				$css['text-align']='center';						
			}
			if ( $textAlign =='center' && $vertical=='top') {
				$css['transform'] = 'rotate('.$rotation.'deg) translate(50%,-50%)';					
		    	$css['transform-origin']='center top';							
		    }elseif( $textAlign =='center' && $vertical=='bottom') {
		    	$css['transform'] = 'rotate('.$rotation.'deg) translate(-50%,50%)';					
		    	$css['transform-origin']='center bottom';
		    	$css['text-align']='right';								
		    }elseif( $textAlign =='right' && $vertical=='middle') {
		    	$css['transform'] = 'rotate('.$rotation.'deg) translate(50%,50%)';							    					
		    	$css['transform-origin']='right center';
		    	$css['text-align']='center';							
		    }elseif( $textAlign =='right' && $vertical=='top') {
		    	$css['transform'] = 'rotate('.$rotation.'deg) translateX(100%)';	
		    	$css['transform-origin']='right top';							    					
		    }elseif( $textAlign =='right' && $vertical=='bottom') {
		    	$css['transform'] = 'rotate('.$rotation.'deg) translateY(100%)';	
		    	$css['transform-origin']='right bottom';							    					
		    	$css['text-align']='right';	
		    }elseif( $textAlign =='left' && $vertical=='middle') {
		    	$css['transform'] = 'rotate('.$rotation.'deg) translate(-50%,-50%)';	
		    	$css['transform-origin']='left center';
		    	$css['text-align']='center';
		    }elseif( $textAlign =='left' && $vertical=='top') {
		    	$css['transform'] = 'rotate('.$rotation.'deg) translateY(-100%)';	
		    	$css['transform-origin']='left top';							    					
		    }elseif( $textAlign =='left' && $vertical=='bottom') {
		    	$css['transform'] = 'rotate('.$rotation.'deg) translateX(-100%)';	
		    	$css['transform-origin']='left bottom';							    					
		    	$css['text-align']='right';	
		    }						
		}else if($rotation == 90){			
    		$rotation =-1* $rotation;    		
			...
			if ( $textAlign =='center' && $vertical=='middle') {
				$css['text-align']='center';						
			}elseif ( $textAlign =='center' && $vertical=='top') {
				$css['transform'] = 'rotate('.$rotation.'deg) translate(-50%,-50%)';					
		    	$css['transform-origin']='center top';
		    	$css['text-align']='right';							
		    }elseif( $textAlign =='center' && $vertical=='bottom') {
	...
	private function _createCSSSDiagonDirection(PHPExcel_Style_Borders $pStyle) {
		$diagon = $pStyle->getDiagonalDirection();		
		$css = array();
		if($diagon ==1 ){
			$css['background']='linear-gradient(to top left,rgba(0,0,0,0) 0%,rgba(0,0,0,0) calc(50% - 0.8px), rgba(0,0,0,1) 50%, 
								rgba(0,0,0,0) calc(50% + 0.8px), rgba(0,0,0,0) 100%)';
		}elseif($diagon ==2 ){
			$css['background']='linear-gradient(to top right,rgba(0,0,0,0) 0%,rgba(0,0,0,0) calc(50% - 0.8px),rgba(0,0,0,1) 50%,
           						rgba(0,0,0,0) calc(50% + 0.8px),rgba(0,0,0,0) 100%)';
		}elseif($diagon ==3 ){
			$css['background']='linear-gradient(to top left,rgba(0,0,0,0) 0%,rgba(0,0,0,0) calc(50% - 0.8px), rgba(0,0,0,1) 50%, 
								rgba(0,0,0,0) calc(50% + 0.8px), rgba(0,0,0,0) 100%),
							  linear-gradient(to top right,rgba(0,0,0,0) 0%,rgba(0,0,0,0) calc(50% - 0.8px),rgba(0,0,0,1) 50%,
           						rgba(0,0,0,0) calc(50% + 0.8px),rgba(0,0,0,0) 100%)';
		}
		return $css;
	}
	...
	private function _createCSSStyleAlignment(PHPExcel_Style_Alignment $pStyle) {
		...
		// Create CSS
		//add start LIXD-79 Phong VNIT-20150911
		$rotation=$pStyle->getTextRotation();
		if($rotation != 0){
			$textAlign = $this->_mapHAlign($pStyle->getHorizontal());	
			$vertical = $this->_mapVAlign($pStyle->getVertical());	
		...
		//add end LIXD-79 Phong VNIT-20150911
...
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	private function _generateTableHeader($pSheet) {
		...
		// Write <col> elements
		$highestColumnIndex = PHPExcel_Cell::columnIndexFromString($pSheet->getHighestDataColumn()) - 1; //Edit LIXD-334 (LIXD-319) TinVNIT 11132015
		$i = -1;
			...
			foreach ($pValues as $cell) {
				...
				if ($cell instanceof PHPExcel_Cell) {
					...
					if ($cell->getValue() instanceof PHPExcel_RichText) {
						...
						//Edit Start - LIXD-472 - TrungVNIT - 2016/29/03
						$dataType = $cell->getDataType();
						$cellValue = $cell->getValue();						
						//edit start BP_LIXD-666 Phong VNIT 20170418
						//if ($this->_preCalculateFormulas ) {
						if($dataType == PHPExcel_Cell_DataType::TYPE_FORMULA)	{							
							//if($dataType == 's'){
								//$cellData = $cellValue;
							//}else{
								//do cong cach tinh cua phpexcel khong dung, nen lay gia tri cua FORMULA tu luc dau load len
								// neu load roi ma set lai gia tri cua cell lien quan trong FORMULA thi cung ko thay doi gia tri cua cell nay
								$CalculatedValue = $cell->getCalculatedValueStatic();
								if(strlen($CalculatedValue) == 0){
								...
								//add end BP_LIXD-666 Phong VNIT 20170418						
						...
						//Edit End - LIXD-472 - TrungVNIT - 2016/29/03
				...
				$colspan = 1;
				$rowspan = 1;
				$id_merge = null; //Add BP_LIXD-667 TinVNIT 04212017
				if (isset($this->_isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow + 1][$colNum])) {
					...
					//Add start BP_LIXD-667 TinVNIT 04212017
					$merge_cells = $pSheet->getMergeCells();
					foreach ($merge_cells as $merge_cell) {
						...
						if($pos_s == $coordinate)
						{
							$id_merge = 'merge_'.$coordinate.'_'.$pos_e;
							break;
						}
					}
					//Add end BP_LIXD-667 TinVNIT 04212017	
				...
				if ($writeCell) {
					...
					// Cell data
					$html .='<span>'. $cellData.'</span>'; //add LIXD-79 Phong VNIT-20150911
	...
	private function _calculateSpans()
	{
		...
		foreach ($sheetIndexes as $sheetIndex) {
			...
			$countColumns = PHPExcel_Cell::columnIndexFromString($sheet->getHighestDataColumn()); //Edit LIXD-334 (LIXD-319) TinVNIT 11132015
