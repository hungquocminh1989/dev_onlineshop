...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
	...
	{
		$base_val = $value; //Add - LIXD-472 - TrungVNIT - 2016/03/25
		...
		if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) { // datetime format
			...
			if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
				...
				if (preg_match('/#?.*\?\/\?/', $format, $m)) {
					...
					if($base_val != '-0'){//Edit - LIXD-472 - TrungVNIT - 2016/03/25
						// scale number
						$value = $value / $scale;
					}
					...
					if (preg_match($number_regex, $m, $matches)) {
						...
						if ($useThousands) {
							if($base_val != '-0'){
								...
								//Add Start -  LIXD-472 - TrungVNIT - 2016/03/25
								$value = preg_replace($number_regex, $value, $format);
								if(preg_match('/^(0+)(\,?)(0*)/',$formatColor)){
									$sprintf_pattern = "%0$minWidth." . strlen($right) . "f";
									$value = sprintf($sprintf_pattern, $value);
									$value = substr_replace($value, ',', strpos($formatColor,','), 0);
								}
							}
							if($value == '-0' && $base_val != '-0'){
								$value = 0;
							}
							//Add End -  LIXD-472 - TrungVNIT - 2016/03/25
						} else {
							if (preg_match('/0([^\d\.]+)0/', $format, $matches)) {
								...
								$value = preg_replace($number_regex, $value, $format);
								//Add Start -  LIXD-472 - TrungVNIT - 2016/03/25
								if(preg_match("/^[0-9]+$/", $value) == 1 && !preg_match('/^0.+$/',$formatColor)) {
