...
class FtpClient implements Countable
{
     ...
     * @return bool
     * @throws \Exception If unable to list the directory to remove
     */
    ...
    public function putFile($local_file,$target_file)
    {
        //$remote_file = basename($local_file);
        $handle      = fopen($local_file, 'r');

        if ($this->ftp->fput($target_file, $handle, FTP_BINARY)) {
            rewind($handle);
            return $this;
        }
    ...
    public function GetFile($local_file,$server_file)
    {
    	$handle = fopen($local_file, 'w');
        if($this->ftp->fget( $handle, $server_file, FTP_BINARY)) {                   
        	rewind($handle);          
            return TRUE;
        }
        return FALSE;
