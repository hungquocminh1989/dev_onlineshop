<?php /* Smarty version Smarty-3.1.14, created on 2017-07-28 18:01:58
         compiled from "E:\DEVELOPMENT\simple_framework\app\template\include\argo_ajax.html" */ ?>
<?php /*%%SmartyHeaderCode:2721597afd8646ecd7-72660171%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '04fe604303606ace42d169f8c6ef8a19b0280c4c' => 
    array (
      0 => 'E:\\DEVELOPMENT\\simple_framework\\app\\template\\include\\argo_ajax.html',
      1 => 1501226642,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '2721597afd8646ecd7-72660171',
  'function' => 
  array (
  ),
  'has_nocache_code' => false,
  'version' => 'Smarty-3.1.14',
  'unifunc' => 'content_597afd86482553_67765181',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_597afd86482553_67765181')) {function content_597afd86482553_67765181($_smarty_tpl) {?><style type="text/css">
...
Argo_ajax.sesstion_err_msg = function() //Edit LIXD-614 TinVNIT 12022016
{
...
// Add start - BP_LIXD-681 - HUNGLM - 20170721
Argo_ajax.prototype.fail_func = function (data) {
	if(data.status == 0){
		if(data.responseText == ""){
			Argo_ajax.message_box('エラーが発生しました', 'サーバーとの接続が切断されました');
		}else{
			Argo_ajax.message_box('エラーが発生しました', data.responseText);
		}
	}else{
		Argo_ajax.message_box('エラーが発生しました', data.responseText);
	}
};	// デフォルト
// Add end - BP_LIXD-681 - HUNGLM - 20170721
...
// クラスメソッド
// メッセージボックス
Argo_ajax.message_box = function (title, msg, closed_func, style) { //Edit ZZZZ-1107 - Tin-VNIT - 2015/06/24
	$('#modal_message').empty();
	$('#modal_message').prepend('<span style="display: block; padding-top: 4px;padding-left: 30px;'+style+'">'+msg+'</span>'); //Edit ZZZZ-1107 - Tin-VNIT - 2015/06/24
...
Argo_ajax.message_box_error = function (title, msg, closed_func, style) { //Edit ZZZZ-1107 - Tin-VNIT - 2015/06/24
	$('#modal_message').empty();
        $('#modal_message').prepend('<span style="display: block; padding-top: 4px;padding-left: 30px;'+style+'">'+msg+'</span>'); //Edit ZZZZ-1107 - Tin-VNIT - 2015/06/24
...
Argo_ajax.message_box_btns = function (title, msg, btns , wth) {
	...
	//Add Start ZZZZ-50 Minh Vnit 2014/12/15
	var width = 400;
	if(wth != null){
		width = wth;
	}
	//Add End ZZZZ-50 Minh Vnit 2014/12/15
...
//Add Start - ZZZZ-66 - TrungVNIT - 2014/12/29
...
//Add End - ZZZZ-66 - TrungVNIT - 2014/12/29

//Add Start - ZZZZ-1133 - TrungVNIT - 2015/06/01
Argo_ajax.getParamURL = function (name) {
    var match = RegExp('[?&]' + name + '=([^&]*)').exec(window.location.search);
    return match && decodeURIComponent(match[1].replace(/\+/g, ' '));
}
//Add End - ZZZZ-1133 - TrungVNIT - 2015/06/01
...
(function($) {
	$.widget("argo.loading", $.ui.dialog, {
        ...
        //Add Start - ZZZZ-41 - TrungVNIT - 2015/02/13
        $.getParam = (function(a) {
            if (a == "") return {};
            var b = {};
        ...
        //Add End - ZZZZ-41 - TrungVNIT - 2015/02/13
})(jQuery);
